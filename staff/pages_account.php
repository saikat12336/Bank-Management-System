...
if (!empty($_FILES["profile_pic"]["name"])) {
    ...
    // Retain old profile picture
    $query = "SELECT profile_pic FROM iB_staff WHERE staff_id=?";
    $stmt_old = $mysqli->prepare($query);
...
//change password
if (isset($_POST['change_password']) && isset($_POST['password']) && isset($_POST['confirm_password'])) {
    $old_password = $_POST['old_password'];
    $new_password = $_POST['password'];
    $confirm_password = $_POST['confirm_password'];
    $admin_id = $_SESSION['staff_id'];

    // Hash the old password to compare with DB
    $hashed_old_password = sha1(md5($old_password));

    // Step 1: Verify the old password
    $query = "SELECT password FROM iB_staff WHERE staff_id = ?";
    $stmt = $mysqli->prepare($query);
    ...
    if ($db_password !== $hashed_old_password) {
    ...
    $err = "New Password cannot be the same as Old Password!";
        ...
        $update_query = "UPDATE iB_staff SET password=? WHERE staff_id=?";
