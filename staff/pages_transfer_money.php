...
$ret_staff = "SELECT name FROM iB_staff WHERE staff_id = ?";
$stmt_staff = $mysqli->prepare($ret_staff);
$stmt_staff->bind_param('i', $staff_id);
$stmt_staff->execute();
$stmt_staff->bind_result($staff_name);
$stmt_staff->fetch();
$stmt_staff->close();
...
if (isset($_POST['deposit'])) {
    ...
    if ($balance === null) $balance = 0;
       ...
       if ($transaction_amt > $balance) {
        ...
        $query = "INSERT INTO iB_Transactions (tr_code, account_id, acc_name, account_number, acc_type, tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone, receiving_acc_no, receiving_acc_name, receiving_acc_holder, staff_id, staff_name) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('sssssssssssssssss', $tr_code, $account_id, $acc_name, $account_number, $acc_type, $tr_type, $tr_status, $client_id, $client_name, $client_national_id, $transaction_amt, $client_phone, $receiving_acc_no, $receiving_acc_name, $receiving_acc_holder, $staff_id, $staff_name);
        $stmt->execute();

        // ✅ Credit Amount to Receiver's Account
$credit_query = "INSERT INTO iB_Transactions (
    tr_code, account_id, acc_name, account_number, acc_type, tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone, receiving_acc_no, receiving_acc_name, receiving_acc_holder, staff_id, staff_name
...
if ($receiver_account_id) {
    ...
    // 📢 Notify Receiver
    $receiver_notification = "An amount of Rs. $transaction_amt has just been credited to your iBank account by $client_name, A/C: $account_number.";
    ...
    $stmt_notify_receiver->execute();
        ...
        $admin_msg = "$client_name has transferred Rs. $transaction_amt from Account No. $account_number to Account No. $receiving_acc_no ( $receiving_acc_holder ), handled by - $staff_name.";
