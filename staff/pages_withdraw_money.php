...
if (isset($_POST['withdrawal'])) {
    ...
    if ($transaction_amt > $amt) {
        ...
        // Insert transaction
        $query = "INSERT INTO iB_Transactions (tr_code, account_id, acc_name, account_number, acc_type, tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone, staff_id, staff_name) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('ssssssssssssss', $tr_code, $account_id, $acc_name, $account_number, $acc_type, $tr_type, $tr_status, $client_id, $client_name, $client_national_id, $transaction_amt, $client_phone, $staff_id, $staff_name);
        $stmt->execute();
...
$admin_msg = "$client_name has withdrawn Rs. $transaction_amt from Bank Account $account_number (Handled By: $staff_name)";
$staff_msg = "$client_name has withdrawn Rs. $transaction_amt from Bank Account $account_number (Handled By: $staff_name)";
$client_msg = "Rs. $transaction_amt has been debited from your iBank Account $account_number.";
...
$stmt_staff->bind_param('is', $client_id, $staff_msg);
$stmt_staff->execute();
        ...
        if ($stmt && $stmt1 && $stmt_staff && $stmt2) {
            ...
            $err = "An error occurred while processing the withdrawal.";
        }
