...
if (isset($_POST['update_client_account'])) {
    ...
    if (!empty($_FILES["profile_pic"]["name"])) {
        ...
        $query = "UPDATE iB_clients SET name=?, national_id=?, phone=?, email=?, address=? WHERE client_id=?";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('sssssi', $name, $national_id, $phone, $email, $address, $client_id);
    }
...
if (isset($_POST['change_client_password'])) {
    if (isset($_POST['current_password'], $_POST['new_password'], $_POST['confirm_password'])) {
        ...
        $query = "SELECT password FROM iB_clients WHERE client_id=?";
        $stmt = $mysqli->prepare($query);
        ...
        // 🔹 Check if password is in old format (MD5 or SHA1)
        if (strlen($stored_password) == 32 || strlen($stored_password) == 40) { 
            // Stored password is using an old hashing method (MD5/SHA1)
            if ($stored_password !== sha1(md5($current_password))) {
                $err = "Current password is incorrect!";
            }
        } else {
            // Stored password is using new hashing (password_hash)
            if (!password_verify($current_password, $stored_password)) {
                $err = "Current password is incorrect!";
            }
        }

        // If the password is correct, proceed to update
        if (!isset($err)) {
            ...
            elseif (
                (strlen($stored_password) == 32 || strlen($stored_password) == 40) &&
                $stored_password === sha1(md5($new_password))
            ) {
                $err = "New password cannot be the same as the old password!";
            } 
            elseif (
                strlen($stored_password) > 50 &&
                password_verify($new_password, $stored_password)
            ) {
                $err = "New password cannot be the same as the old password!";
            } else {
                ...
                if ($stmt->execute()) {
                    ...
                    // ✅ Convert old password format to new format after update
...
.nav-pills .nav-link {
    color: #fefafa7d; /* Normal font color */
    font-weight: 500;
...
.nav-pills .nav-link:hover {
    background-color: #ebe4fb; /* Light purple background on hover */
    color: #4b2c91;
}
