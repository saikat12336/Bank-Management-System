...
while ($row = $res->fetch_object()) {
  ...
  // If handled by staff, prepare the note
// Determine handler label
if (!empty($row->admin_id)) {
  $handledBy = "<br><small class='text-muted'>Handled by: Admin</small>";
} elseif (!empty($row->staff_id)) {
  // Show staff name if available, else just "Staff"
  $staffName = !empty($row->staff_name) ? $row->staff_name : 'Staff';
  $handledBy = "<br><small class='text-muted'>Handled by: $staffName</small>";
} elseif (!empty($row->client_id)) {
  $handledBy = "<br><small class='text-muted'>Handled by: Client</small>";
}

// Set badge color
if ($row->tr_type == 'Deposit') {
  $alertClass = "<span class='badge badge-success'>{$row->tr_type}</span>{$handledBy}";
} elseif ($row->tr_type == 'Withdrawal') {
  $alertClass = "<span class='badge badge-danger'>{$row->tr_type}</span>{$handledBy}";
} else {
  $alertClass = "<span class='badge badge-warning'>{$row->tr_type}</span>{$handledBy}";
}
...
?>
  <tr>
    ...
    <!-- Account Number + Receiver Acc No (if Transfer) -->
    <td>
      <?php echo $row->account_number; ?>
      <?php if ($row->tr_type == 'Transfer') { ?>
        <br><small><strong>To:</strong> <?php echo $row->receiving_acc_no; ?></small>
      <?php } ?>
    </td>
    ...
    <!-- Client Name + Receiver Name (if Transfer) -->
    <td>
      <?php echo $row->client_name; ?>
      <?php if ($row->tr_type == 'Transfer') { ?>
        <br><small><strong>To:</strong> <?php echo $row->receiving_acc_holder; ?></small>
      <?php } ?>
    </td>
