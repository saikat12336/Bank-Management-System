...
if (isset($_POST['change_staff_password'])) {
    ...
    // Get staff number from GET
    $staff_number = $_GET['staff_number'] ?? '';

    // Step 1: Get staff_id and current password from DB
    $get_query = $mysqli->prepare("SELECT staff_id, password FROM iB_staff WHERE staff_number=?");
    $get_query->bind_param('s', $staff_number);
    $get_query->execute();
    $get_query->bind_result($staff_id, $db_password);
    $get_query->fetch();
    $get_query->close();
