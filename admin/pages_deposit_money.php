...
if (isset($_POST['deposit'])) {
    ...
    //Notication
$notification_admin_staff = "$client_name Has Deposited Rs. $transaction_amt To Bank Account $account_number Handled by Admin.";
$notification_client = "The amount Rs. $transaction_amt has been credited to your iBank Account $account_number. Thank you for banking with us!";

// Insert transaction
$query = "INSERT INTO iB_Transactions (tr_code, account_id, acc_name, account_number, acc_type, tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone, admin_id) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
$stmt = $mysqli->prepare($query);
$stmt->bind_param('sssssssssssss', $tr_code, $account_id, $acc_name, $account_number, $acc_type, $tr_type, $tr_status, $client_id, $client_name, $client_national_id, $transaction_amt, $client_phone, $admin_id);
$stmt->execute();

// Admin Notification
$admin_note = $mysqli->prepare("INSERT INTO iB_notifications (notification_for, client_id, notification_details) VALUES ('admin', ?, ?)");
$admin_note->bind_param('is', $client_id, $notification_admin_staff);
$admin_note->execute();

// Staff Notification
$staff_note = $mysqli->prepare("INSERT INTO iB_notifications (notification_for, client_id, notification_details) VALUES ('staff', ?, ?)");
$staff_note->bind_param('is', $client_id, $notification_admin_staff);
$staff_note->execute();

// Client Notification
$client_note = $mysqli->prepare("INSERT INTO iB_notifications (notification_for, client_id, notification_details) VALUES ('client', ?, ?)");
$client_note->bind_param('is', $client_id, $notification_client);
$client_note->execute();

if ($stmt && $admin_note && $staff_note && $client_note) {
                                    ...
                                    <form method="post" enctype="multipart/form-data" role="form">
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                    ...
                                                    ?>
                                                    <input type="hidden" name="tr_code" readonly value="<?php echo $_transcode; ?>" required class="form-control" id="exampleInputEmail1">
                                                
                                                <div class=" col-md-6 form-group">
    <label for="exampleInputPassword1">Amount Deposited(Rs.)</label>
    <input type="number" name="transaction_amt" step="0.5" min="1" required class="form-control" id="exampleInputEmail1">
</div>
